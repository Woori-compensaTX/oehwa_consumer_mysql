...
public class WonhwaConsumer {
    ...
    // 원화 입금 토픽 자동 생성 (파티션 2, 레플리카 2)
    @Bean
    public NewTopic WonhwaDeposit() {
        return TopicBuilder.name("ServerWonhwaDeposit")
                .partitions(2) // 파티션 2개
                .replicas(2)   // 레플리카 2개 (브로커 2대일 때만 가능)
                .build();
    }

    // 원화 출금 토픽 자동 생성 (파티션 2, 레플리카 2)
    @Bean
    ...
    // 원화 입금 메시지 소비 (4회 재시도, DLQ 자동 이관, 지수 백오프)
    ...
    // 원화 출금 메시지 소비 (4회 재시도, DLQ 자동 이관, 지수 백오프)
